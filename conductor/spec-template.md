...
## Test Strategy
- [Primary automated tests that will prove the contract for this track]
- [Failure modes or regressions that must be covered, not just the happy path]
- [Where integration tests are required instead of heavily mocked unit tests]
- [If UI is involved, at least one visible-behavior test and one manual verification step]
...
- [ ] [Criterion 3 - automated tests cover at least one relevant failure or negative path]
...
## Test Strategy
- Integration test the seed script and lookup logic against a real test database
- Unit test fuzzy matching with known ABA/eBird divergence cases
- Add at least one negative-path test covering unknown species fallback behavior
- For UI wiring, verify the running app visibly reflects live ABA data instead of only asserting helper function output
...
- [ ] Automated tests cover the unknown-species fallback path
